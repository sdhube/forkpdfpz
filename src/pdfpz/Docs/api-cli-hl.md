...
`F_SANITIZE_INFO` onward. This is the user manually telling `cli.py`
where to start -- e.g. because they know those earlier steps already
ran -- not `BookOperationPlan` detecting that itself; see the resume
sequence further down for the DB-driven version:
...
flowchart TD
    ...
    Loop -->|K_PRINT_FIRST| S11["actions.print_first_entry()"]
    S11 --> D11["state.mark_done(...)"] --> Loop
    Loop -->|"None (state.is_finished())"| Done(["Pipeline complete (started from sanitize_info)"])
```
